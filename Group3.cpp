...
bool Group3::follow(const GameStatus &gstat, CardSet &cs) {
    ...
    leadSize = pile.size();
     ...
     if (hantei == 0 && judgeStrongestNum(hand) && (leadSize==0 && leadSize==1)) {
      hantei = 1;
    }
    if (hantei == 1) {
      return follow2(gstat, cs);
    }
      ...
      for (int i = 0; i < hand.size() - 2; i++) {
        ...
        if(tmp1.rank() == tmp3.rank()){
          ...
          // jkrを含めた4枚が最強なら出す
            ...
            if(jkrumu == 1 && strongest(memory, tmp1, 4)) {
      ...
      for (int i = 0; i < hand.size() - 1; i++) {
        ...
        if(tmp1.rank() == tmp2.rank()){
          ...
          if(strongest(memory, tmp1, 2)) {
            if((tmp1.rank() >= 3 && tmp1.rank() <= 11) || hand.size() <= 3) {
          // 弱い2枚を出す
    ...
    if(leadSize == 4){ // リードが4枚の場合
      for (int i = 0; i < hand.size() - 3; i++) {
        ...
        if(tmp1.rank() == tmp4.rank()){
          if (tmp4.isGreaterThan(pile.at(0)) || tmp4.isGreaterThan(pile.at(1))) { // by Teigo Nakamura 2020/07/04
            // if((tmp1.rank() >= 2 && tmp1.rank() <= 11) || hand.size() <= 5) {
              ...
              cs.insert(tmp3);
              cs.insert(tmp4);
              hand.remove(tmp1); // カード集合を remove
              hand.remove(tmp2);
              hand.remove(tmp3);
              hand.remove(tmp4);
      ...
      for (int i = 0; i < hand.size() - 2; i++) {
        ...
        if(tmp1.rank() == tmp3.rank()){
          if (tmp3.isGreaterThan(pile.at(0)) || tmp3.isGreaterThan(pile.at(1))) { // by Teigo Nakamura 2020/07/04
            // if(strongest(memory, tmp1, 3)) {
              cs.clear();
              cs.insert(tmp1);
              cs.insert(tmp2);
              cs.insert(tmp3);
    ...
    else if(leadSize == 2){ // リードが2枚以上の場合
      for (int i = 0; i < hand.size() - 1; i++) {
        tmp1 = hand.at(i);
        tmp2 = hand.at(i + 1);
        if(tmp1.rank() == tmp2.rank()){
      ...
      if(hand.size() >= 8) { // 手札が8枚を切るまでは最強の一枚（jkrを持っていたらjkr）を温存
        for (int i = 0; i < hand.size() - 1; i++) {
          tmp1 = hand.at(i);
          if (tmp1.isGreaterThan(pile.at(0)) && hand.at(i).rank() != hand.at(i+1).rank() && hand.at(i).rank() != hand.at(i-1).rank()) {
            cs.insert(tmp1);           
            hand.remove(tmp1);
            return true;
          }
      ...
      else if(hand.size() >= 7) { // 手札が７枚を切るまでは最強の一枚（jkrを持っていたらjkr）を温存
        for (int i = 0; i < hand.size() - 1; i++) {
          tmp1 = hand.at(i);
...
int Group3::count(const CardSet cset, int rank) {
  int i, n=0;
...
bool Group3::strongest(const CardSet cset, Card c, int num) {
  int i;
  Card temp;
...
bool Group3::judgeStrongestNum(const CardSet cset) {
  int n = 0;
  for (int i=0; i<cset.size(); i++) {
    if(strongest(memory, cset.at(i), 1)) {
    n++;
    }
  }

  return (n >= cset.size()/2);
}

bool Group3::follow2(const GameStatus &gstat, CardSet &cs) {
    CardSet pile(gstat.pile);
    Card tmp;
    int leadSize;
    ...
    if (leadSize == 0) { // 自分がリーダなら
      if (judgeStrongestNum(hand)) {
    tmp = hand.at(hand.size()-1);
    cs.insert(tmp);
    hand.remove(tmp);
    return true;
      } else {
